...
fromList :: forall n m a. (KnownNat n, KnownNat m, n ~ (1 + m)) => [a] -> Vec n a
fromList xs
  | Prelude.length xs == l = UnsafeMkVec $ V.fromList xs
  | otherwise              = error "length of list not match vector"
  where l = fromIntegral $ natVal (Proxy @n)

toList :: forall n a. (KnownNat n) => Vec n a -> [a]
toList vec = V.toList $ getVector vec
